...
import { Search, ChevronDown, ChevronLeft, ChevronRight, TrendingUp, User } from "lucide-react";
    ...
    const loadLeaderboardData = async () => {
        ...
        `)
            ...
            if (leaderboardData.length < minEntries) {
                ...
                const startingValues = [158166.04, 32997.16, 39365.16, 4.15, 64992.47, 44874.09, 68116.98, 97756.09, 93865.73, 38812.35];
                const pnls = [6274.79, 125.96, 96.44, 74.93, 68.87, 125.84, 53.86, 41.60, 30.91, 38.65];
                const volumes = [648632.27, 330.84, 1034.99, 752.12, 1393.01, 755.98, 364.71, 161.65, 174.32, 181.40];
                            ...
                            <div className="absolute right-0 mt-2 w-32 bg-sectionBg border border-btnBorder rounded-lg shadow-xl py-2 z-50">
                                {(["24H", "7D", "30D", "ALL"] as TimePeriod[]).map((period) => (
                                    <button
                                        ...
                                        className={`w-full text-left px-4 py-2 text-sm transition-colors ${timePeriod === period
                        ...
                        <>
                                ...
                                <table className="w-full">
                                    ...
                                    <tbody>
                                        {paginatedEntries.map((entry) => {
                                            const hasUser = entry.trader && entry.trader !== "Unknown";
                                                ...
                                                >
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <div className="flex items-center gap-2">
                                                            ...
                                                            <span className="text-white">
                                                                {hasUser ? formatWalletAddress(entry.trader) : entry.trader}
                                                            </span>
                                                        </div>
                                                    </td>
                                                    <td className="px-6 py-4">
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <div className="flex flex-col">
                                                            <span
                                                                className={`font-medium ${entry.pnl >= 0 ? "text-green" : "text-red-400"
                                                                    }`}
                                                            >
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <div className="flex items-center gap-2">
                                                            <span
                                                                className={`font-medium ${entry.roi >= 0 ? "text-green" : "text-red-400"
                                                                    }`}
                                                            >
                            ...
                            <div className="px-6 py-4 border-t border-btnBorder">
                                <p className="text-xs text-slate-400">
                                    Excludes accounts with less than 10M USDC account value and less than 10M USDC trading volume. ROI = PNL / max(10M
                                    USDC, starting account value) ∙ (maximum net deposits for the time window)
                                </p>
                            </div>
                            ...
                            <div className="flex items-center justify-between px-6 py-4 border-t border-btnBorder">
                                <div className="text-sm text-slate-400">
                                    Rows per page: {itemsPerPage} · {startIndex + 1}-{Math.min(endIndex, sortedEntries.length)} of {sortedEntries.length}
                                </div>
                                <div className="flex items-center gap-2">
                                    <button
